...
impl<Trail, Domain> Empty for TrailMemory<Trail, Domain> where
 Trail: Empty
{
...
impl<Trail, Domain> Push<Back> for TrailMemory<Trail, Domain> where
...
impl<Trail, Domain> Replace for TrailMemory<Trail, Domain> where
 Trail: TrailVariable + AssociativeCollection<Location=usize, Item=Domain>,
 Domain: Clone
{
...
impl<Trail, Domain> Display for TrailMemory<Trail, Domain> where
 Domain: Display
{
...
impl<Trail, Domain> Freeze for TrailMemory<Trail, Domain> where
...
impl<Trail, Domain> Snapshot for FrozenTrailMemory<Trail, Domain> where
 Trail: TrailRestoration + Collection<Item=Domain>
{
  ...
  fn restore(mut self, label: Self::Label) -> Self::State {
    self.store.trail.undo(label, self.store.variables.deref_mut());
    self.store
  }
}
